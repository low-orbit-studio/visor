...
Visor uses a two-layer distribution model — tokens via npm (`@loworbitstudio/visor`) and components via a shadcn-style copy-and-own registry. Each layer has a distinct versioning approach.

---

## Tokens Package — Semver (`@loworbitstudio/visor`)
...
1. Select which packages are affected (`@loworbitstudio/visor`)
