...
// - VisorColors registers as a ThemeExtension.
...
void main() {
  group('VisorAppTheme (generated)', () {
    test('light returns Material 3 ThemeData with VisorColors extension', () {
      final theme = VisorAppTheme.light;
      expect(theme.useMaterial3, isTrue);
      expect(theme.brightness, Brightness.light);
      expect(theme.extension<VisorColors>(), isNotNull);
    });

    test('dark returns Material 3 ThemeData with VisorColors extension', () {
    ...
    test('primary anchor (#6952D9) propagates into light ColorScheme', () {
      ...
      expect(light.colorScheme.primary, UIColors.light.interactivePrimaryBg);
      expect(
        UIColors.primary500,
    ...
    test('scaffold background matches surfacePage', () {
      expect(
        ...
        UIColors.light.surfacePage,
      );
      expect(
        VisorAppTheme.dark.scaffoldBackgroundColor,
        UIColors.dark.surfacePage,
      );
    });

    test('generated UIColors exposes primitive shade scale', () {
      // Full-scale roles (primary, accent, neutral) emit 50..950.
      expect(UIColors.primary50, isA<Color>());
      expect(UIColors.primary950, isA<Color>());
      expect(UIColors.neutral50, isA<Color>());
      expect(UIColors.accent600, isA<Color>());
    });

    test('pre-computed opacity variants carry correct alpha', () {
      // 10% alpha = 0x1A / 255 ≈ 0.1
      expect(UIColors.primary600_10o.a, closeTo(0.1, 0.02));
      // 50% alpha = 0x80 / 255 ≈ 0.5
      expect(UIColors.primary600_50o.a, closeTo(0.5, 0.02));
    });
