...
export interface TextareaProps
   ...
   * credential or notes-style field where autofill is desired. Browsers
   * ignore `autocomplete="off"` on individual fields, so `"ignore"` emits
   * the per-manager data-* attributes (`data-1p-ignore`, `data-bwignore`,
...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, size, passwordManagers = "ignore", ...props }, ref) => {
    const ignoreAttrs =
