...
2. **Tokens** — `@loworbit/visor-tokens` npm package. CSS custom properties for design consistency. Updated via `npm update`.
...
`@loworbit/visor-tokens` is imported once in the global CSS entry point. Do not import it in individual component files.
...
your-project/
├── app/
│   └── globals.css          ← @import "@loworbit/visor-tokens" lives here
├── components/
│   └── ui/
│       ├── button/
...
- Do not modify files in `node_modules/@loworbit/visor-tokens` — override tokens in your global CSS instead.
- Do not import `@loworbit/visor-tokens` in individual component files — it belongs in the global CSS entry point only.
