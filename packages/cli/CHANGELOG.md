...
- 76323bc: Badge ground and ink tokens (VI-661).
...
- Updated dependencies [938b39a]
- Updated dependencies [938b39a]
  - @loworbitstudio/visor-theme-engine@0.20.0

## 1.26.0

### Minor Changes

- 22b3e02: VI-643: `Button` takes a `pending` prop, so a consumer never hand-rolls a busy state again.
...
- 444a4f5: VI-643 follow-up: `pending` and `gated` now stop a FORM SUBMIT, not only an `onClick`.
...
- 0b0cfd7: VI-638: `typography.scale` now multiplies the `--font-size-*` ramp it always claimed to, and it is the only mechanism that sets type size.
...
- 3c9b72b: VI-639: `--font-weight-semibold` is a step on the weight ramp again, not the theme's heading weight, and the `weights` array a theme declares is now authoritative.
...
- 2d74eef: VI-636: `phone-input` no longer hands `intl-tel-input` an explicit `undefined` for its two optional pass-through props.
...
- 6149588: VI-631: The composition lint — `visor check design` and `visor check diff` gain the three assertions they were missing, so a surface can be **proven** to introduce no styling outside the project's kit.
...
- ffdcd97: Theme-bindable component tokens for the admin-UI families (VI-625).
...
- Updated dependencies [ffdcd97]
  - @loworbitstudio/visor-theme-engine@0.19.0
