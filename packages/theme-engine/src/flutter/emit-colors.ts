/**
 * Flutter adapter — emit ui_colors.dart.
 *
 * Generates a Dart file exposing:
 * - Primitive color constants for every generated shade scale
 *   (`UIColors.primary50..950`, `UIColors.neutral50..950`, etc.)
 * - Pre-computed opacity variants on primary/accent (matches Low Orbit
 *   convention of compile-time opacity — see SoleSpark's `UIColors`)
 * - `VisorColors.light` and `VisorColors.dark` instances constructed from
...
export function emitColorsDart(input: AdapterInput): string {
  ...
  lines.push("sealed class UIColors {");
  ...
  lines.push("  // ========================================================================");
  lines.push(`  static final VisorColors light = ${semanticInstance(tokens, "light")};`);
  lines.push("");
  lines.push("  // ========================================================================");
  lines.push("  // Semantic — Dark");
  lines.push("  // ========================================================================");
  lines.push(`  static final VisorColors dark = ${semanticInstance(tokens, "dark")};`);
  lines.push("}");
  return lines.join("\n") + "\n";
}

/** Build a `VisorColors(...)` expression for light or dark mode. */
function semanticInstance(
  tokens: SemanticTokens,
  mode: "light" | "dark",
): string {
  const parts: string[] = ["VisorColors("];
  const emit = (
    field: string,
