...
- VI-453: `nextjs` adapter now emits the full `visor-semantic` cascade layer —
  intent aliases (`--primary`, `--accent`, `--success`, `--warning`,
  `--destructive`, `--info`, `--primary-text`), hairline aliases (`--hairline`,
  `--hairline-strong`), and discrete pixel-scale aliases (`--text-{11..48}`,
  `--space-{1..16}`) using the same generators the `docs` adapter consumes.
  Mode scoping mirrors the docs adapter: `html:not(.dark)` for light,
  `html.dark` (+ `prefers-color-scheme: dark`) for dark; discrete scales emit
  unconditionally in `:root` / `scopePrefix`.

  **Consumer impact:** themes generated by `npx visor add theme` now ship a
  populated `visor-semantic` layer. Consumers who declared a bridge `:root`
  block to fill these aliases (e.g. `prototype-overlay.css`) can delete that
  block after regenerating their theme CSS with `@loworbitstudio/visor-theme-engine ^0.15.0`.
