...
import { DataTableRowActionsDemo } from '@/components/patterns/data-table-row-actions-demo';

## Preview

<BlockPreview title="Data Table Row Actions" code={"<DataTableRowActionsDemo />"}>
  <DataTableRowActionsDemo />
</BlockPreview>
...
- [Confirm Dialog](/docs/components/confirm-dialog)
- [Toast](/docs/components/toast)
- [Bulk Action Bar](/docs/components/bulk-action-bar)
...
- For search, category filters, and sortable columns on a read-heavy list, use the [Data Table with Filters](/docs/patterns/data-table-with-filters) pattern — the two compose (filter toolbar above, row actions per row).
