...
`@loworbit/visor-tokens` is the only npm-distributed piece of Visor. It follows [Semantic Versioning](https://semver.org/).
...
After completing work that affects `@loworbit/visor-tokens`, run:
...
- [Release Notes](/docs/releases) — full changelog for `@loworbit/visor-tokens`
