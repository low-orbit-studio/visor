...
| `--button-icon-size` | Icon-only (`size="icon"`) glyph size. The square box is this plus `icon-pad` on every side. | `1rem` |
| `--button-icon-pad` | Icon-only hit-target pad around the glyph, on every side. | `var(--spacing-2, 0.5rem)` |
| `--button-icon-radius` | Icon-only corner rounding (`999px` for a round mark). | `var(--button-radius, var(--radius-md, 0.375rem))` |
| `--button-icon-ghost-color` | Icon-only ghost ink at rest. Hover comes up to `--text-primary`. | `var(--text-secondary, #6b7280)` |
