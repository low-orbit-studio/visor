...
before it existed. Case and tracking fall back to "as if unset": `inherit` on
the label and Text elements, and `revert-layer` on Button, whose `<button>`
already carries a browser `normal` / `none`.
