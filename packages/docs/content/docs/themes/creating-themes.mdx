...
The engine emits a 6-tier letter-spacing ramp from the theme's `typography.letter-spacing`
YAML block, falling back to the Visor default ramp per tier:

| Token | Default | Notes |
| --- | --- | --- |
| `--letter-spacing-xl` | `0.16em` | Widest — eyebrows / overlines, uppercase |
| `--letter-spacing-lg` | `0.1em` | |
| `--letter-spacing-md` | `0.05em` | Anchor — body / default |
| `--letter-spacing-sm` | `0.025em` | |
| `--letter-spacing-xs` | `0.01em` | |
| `--letter-spacing-tight` | `-0.01em` | Tightest — large display headings |

```yaml
typography:
  letter-spacing:
    xl: "3.5rem" # any subset of tiers; omitted tiers use the default ramp
    tight: "0.03rem"
```

The legacy triad keys stay valid for back-compat and fold onto the ramp (`normal`→`md`,
`wide`→`lg`, `tight`→`tight`). The engine also emits `--letter-spacing-normal` (= `md`) and
`--letter-spacing-wide` (= `lg`) as aliases so older consumers keep resolving.
