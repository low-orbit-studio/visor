...
## Single-Mode Brands (`color-scheme`)

Some brands only ship one mode. A theme can declare this in its `.visor.yaml` with the `color-scheme` field, and the engine generates mode-correct CSS:

```yaml
name: Animal
color-scheme: dark-only   # dark-only | light-only | adaptive (default)
colors:
  primary: "#e8b64c"
```
...
|-------|------------------|
| `adaptive` *(default)* | Light palette on `:root`, dark behind `@media (prefers-color-scheme: dark)` + manual toggles. |
| `dark-only` | Dark palette pinned directly on the host selector, plus `color-scheme: dark`. **No** `prefers-color-scheme` media query and **no** light blocks. |
| `light-only` | Light palette pinned on the host, plus `color-scheme: light`. No dark blocks. |

For a single-mode brand the palette is unconditional — there is nothing to toggle — so the `.dark` / `.light` selectors and the `prefers-color-scheme` fallback are omitted entirely. The emitted `color-scheme` property also tells the browser to render native UI chrome (form controls, scrollbars) in the matching mode.

This makes the "light-at-`:root`, dark-behind-`prefers`" shape ungeneratable for a `dark-only` brand — the failure mode where a dark brand briefly (or permanently, on a light OS) renders as a white app.

## Form-Control Edges (`edges`)

One line turns off the resting edge of every form control — Input, Textarea, Select, Checkbox, Switch, TagInput, Chip, FileUpload, EmptyState and the edged Button faces — dashed edges included:

```yaml
# my-theme.visor.yaml
edges: off   # on (default) | off
```

`edges: off` emits `--control-edge-width: 0`. Nothing moves when it flips, because the edge is an inset `outline`, not a `border`. Focus and invalid stay visible, and an unchecked checkbox takes a fill so it still reads as a box. To scope it to one part of an app, set `--control-edge-width: 0` on any ancestor. The full token set is in [Component Tokens → `control`](/docs/themes/component-tokens#control).
...
/* Element baseline — form-control font inheritance, box-sizing, UA
   margin/appearance normalisation. Opt-in: skip it if you already ship
   Tailwind preflight or your own reset. */
