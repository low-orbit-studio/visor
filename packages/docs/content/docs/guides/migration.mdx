...
> **Note:** `@loworbitstudio/visor-core` is not yet published to npm. For internal projects with monorepo access, use a workspace-relative import. For new standalone projects, use `npx @loworbitstudio/visor init --template nextjs` which generates tokens inline and requires no npm package.
