...
For an editorial-density composition, see [`/showcases/admin-wizard`](/showcases/admin-wizard) — a four-step ENTR-style create-organization flow that exercises the block against admin-ui-density form rows using `Field` family primitives, `Select`, repeating invite rows, per-step validation, and the cancel guard.
