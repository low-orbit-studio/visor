...
- **Header scale** — `headerSize`, `titleSize`, `titleFamily`, `leading` pass straight through to the internal `PageHeader` so you can tune editorial scale per-recipe without forking. All four are optional; omitting them keeps the default header rendering.
