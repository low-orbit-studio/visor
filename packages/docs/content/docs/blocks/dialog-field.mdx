...
| `DialogFieldControl` | The recessed control well. Props: `size` (`sm \| md`, default `md`) tunes the well padding only — the label typography is unchanged, matching the input atom's `size` semantics; `icon` (leading) and `trailing` (control/caret). Any `input` / `select` / `textarea` dropped inside renders on the well surface. |
