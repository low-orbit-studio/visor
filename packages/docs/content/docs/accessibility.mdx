...
Since components use CSS custom properties from `@loworbit/visor-tokens`, contrast is defined at the token level. Check token values when authoring new themes.
