...
Since components use CSS custom properties from `@loworbitstudio/visor`, contrast is defined at the token level. Check token values when authoring new themes.
