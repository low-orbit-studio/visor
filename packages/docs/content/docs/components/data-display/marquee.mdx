...
<ComponentPreview
  ...
  code={"import { Marquee } from '@/components/ui/marquee/marquee';\n\nconst clients = ['Acme Corp', 'Vercel', 'Low Orbit Studio', 'Figma', 'Notion', 'Linear'];\n\n<Marquee\n  bands={[\n    { items: clients, direction: 'left', durationSec: 25, separator: '●' },\n    { items: [...clients].reverse(), direction: 'right', durationSec: 30, separator: '○' },\n  ]}\n  pauseOnHover\n  gap=\"2.5rem\"\n/>"}
>
  <Marquee
    bands={[
      {
...
| `durationSec` | `number` | `25` | Duration of one full cycle in seconds |
