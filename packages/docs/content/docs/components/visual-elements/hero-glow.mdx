...
When `prefers-reduced-motion: reduce` is active, the breathing animation is disabled. The glow rests at full opacity (`1`) and scale (`1`) — still fully visible, just not animated. Full opacity is faithful to the `bl-hero-glow` origin (BL-326), so a pixel-accurate adoption needs no consumer override.
