---
title: Heading
description: A semantic heading component (h1–h6) with independent size and weight variants. Decouples visual presentation from semantic level.
---

import { ComponentPreview } from '@/components/preview';
import { PropsTable } from '@/components/props-table';
import { Heading } from '@/components/ui/heading';

## Levels

Each `level` renders the corresponding HTML heading element (`<h1>` through `<h6>`) with an auto-mapped visual size.

<ComponentPreview
  title="All Levels"
  code={"import { Heading } from '@/components/ui/heading/heading';\n\n<Heading level={1}>Heading 1</Heading>\n<Heading level={2}>Heading 2</Heading>\n<Heading level={3}>Heading 3</Heading>\n<Heading level={4}>Heading 4</Heading>\n<Heading level={5}>Heading 5</Heading>\n<Heading level={6}>Heading 6</Heading>"}
>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
    <Heading level={1}>Heading 1</Heading>
    <Heading level={2}>Heading 2</Heading>
    <Heading level={3}>Heading 3</Heading>
    <Heading level={4}>Heading 4</Heading>
    <Heading level={5}>Heading 5</Heading>
    <Heading level={6}>Heading 6</Heading>
  </div>
</ComponentPreview>

## Size Override

The `size` prop lets you decouple visual size from semantic level — an `<h3>` can look like an `<h1>`.

<ComponentPreview
  title="Size Override"
  code={"<Heading level={3} size=\"2xl\">h3 that looks like h1</Heading>\n<Heading level={1} size=\"sm\">h1 that looks like h5</Heading>"}
>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
    <Heading level={3} size="2xl">h3 that looks like h1</Heading>
    <Heading level={1} size="sm">h1 that looks like h5</Heading>
  </div>
</ComponentPreview>

## Weights

`weight="heading"` is the default and follows the active theme's declared heading
weight (`--font-weight-heading`). The other four are steps on the named ramp,
which each theme resolves against the font weights it actually loads — so on a
theme that ships no face between 500 and 700, `semibold` resolves to 700 rather
than to a weight the browser would have to substitute.

Reach for `heading` when you want "whatever this theme's headings weigh", and a
ramp step when you want a specific weight relative to body copy.

<ComponentPreview
  title="Weight Variants"
  code={"<Heading weight=\"heading\">Heading Weight (theme default)</Heading>\n<Heading weight=\"normal\">Normal Weight</Heading>\n<Heading weight=\"medium\">Medium Weight</Heading>\n<Heading weight=\"semibold\">Semibold Weight</Heading>\n<Heading weight=\"bold\">Bold Weight</Heading>"}
>
  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
    <Heading weight="heading">Heading Weight (theme default)</Heading>
    <Heading weight="normal">Normal Weight</Heading>
    <Heading weight="medium">Medium Weight</Heading>
    <Heading weight="semibold">Semibold Weight</Heading>
    <Heading weight="bold">Bold Weight</Heading>
  </div>
</ComponentPreview>

## Installation

```bash
npx visor add heading
```

This copies two files into your project:

- `components/ui/heading/heading.tsx` — the component
- `components/ui/heading/heading.module.css` — the styles

## Usage

```tsx
import { Heading } from '@/components/ui/heading/heading';

export default function Example() {
  return <Heading level={1}>Page Title</Heading>;
}
```

## API Reference

### HeadingProps

<PropsTable component="heading" />

The component also accepts all standard HTML heading attributes.

## Accessibility

- Always use headings in a logical hierarchy — don't skip levels (e.g. `h1` → `h3`). Use the `size` prop to adjust visual appearance without changing the semantic level.
- Every page should have exactly one `<h1>`. Use `level={1}` for the main page title only.
- Screen readers navigate by heading structure. Meaningful, descriptive heading text improves navigation for keyboard and AT users.
- Avoid using `Heading` purely for visual styling — use `Text` instead if semantics don't apply.

## Em accent

Editorial headings often accent one phrase with brand color — rendered as non-italic, colored text via `<em>`. This is a recurring pattern across Visor consumers and has a Visor-sanctioned answer: one theme-level rule, applied once.

**The pattern:** Add this rule to your theme or global stylesheet:

```css
em {
  color: var(--text-link);
  font-style: normal;
}
```

Then use bare `<em>` in any heading (or body text):

```tsx
<Heading level={1}>Build faster <em>with Visor</em></Heading>
```

**Why `--text-link`:** It's the most universally available accent token — all Visor themes define it, it passes contrast against typical heading backgrounds, and it maps intuitively to "highlighted" text. You can swap it for any token that fits your theme:

| Token | When to use |
|-------|-------------|
| `--text-link` | Default recommendation — themed link/accent color |
| `--accent` | Bare intent alias; resolves to your theme's accent hue |
| `--primary` | Bare intent alias; resolves to your theme's primary hue |

**Scope the rule to headings if needed:** If `font-style: normal` on all `<em>` would conflict with your body copy, scope it:

```css
h1 em, h2 em, h3 em, h4 em, h5 em, h6 em {
  color: var(--text-link);
  font-style: normal;
}
```

This rule is intentionally **not** built into `@loworbitstudio/visor-core` — it's an opt-in convention. Visor doesn't reset `<em>` globally; you apply it once in the stylesheet that owns your theme.

## Customization

After copying the component, you own it completely. Common customizations:

```tsx
// Add a gradient or custom color
<Heading level={1} style={{ background: 'linear-gradient(...)', WebkitBackgroundClip: 'text' }}>
  Gradient Title
</Heading>

// Combine with a custom class for one-off styles
<Heading level={2} className="my-section-title">Section</Heading>
```
