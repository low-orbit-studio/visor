...
| `--scope-prefix <selector>` | (nextjs) Wrap all rules under a CSS selector (e.g. `body.blacklight-theme`) so multiple themes can coexist via body-class repaint. Dark-mode block scopes to `<selector>.dark`. Default: `:root`. |
| `--json` | Output structured JSON (for AI agents) |
