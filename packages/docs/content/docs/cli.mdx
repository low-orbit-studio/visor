...
| `--theme <slug-or-path>` (required) | Theme slug (looked up in `themes/` then `custom-themes/`) or path to a `.visor.yaml` |
...
npx @loworbitstudio/visor sandbox approve --name org-mgmt --diff
```

Shells out to a sandbox-local Playwright install and captures full-page screenshots of every route into `captures/approved/<route-slug>.png`. With `--diff`, captures land in `captures/pending/` first; only routes that pixel-differ from their approved baseline produce a diff PNG in `captures/diffs/`. Iterations stay traceable — operators see only what changed.

| Flag | Description |
|------|-------------|
| `--name <name>` (required) | Sandbox name (from a prior `sandbox init`) |
| `--diff` | Pixel-diff against the prior approved baseline instead of overwriting |
| `--json` | Emit structured JSON for AI agents |
